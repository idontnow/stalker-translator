...
LANG = {
    "eng": {
        "PM_TITLE": "=== PROJECT MANAGER ===", "FOUND_PROJ": "Found projects:", "NO_PROJ": "No projects found. Please run ST_Setup.exe", "NO_PROJ_FOLDER": "No 'projects' folder. Run ST_Setup.exe",
        "NEW_PROJ": "[N] Create New Project (Run Setup)", "QUIT": "[Q] Quit", "SELECT": "👉 Select project", "INVALID": "Invalid choice.", "CREATE_HINT": "To create a new project, run 'ST_Setup.exe'", "CLOSING": "Closing...",
    ...
    "ltu": {
        ...
        "NEW_PROJ": "[N] Kurti naują projektą (Setup)", "QUIT": "[Q] Išeiti", "SELECT": "👉 Pasirinkite projektą", "INVALID": "Neteisingas pasirinkimas.", "CREATE_HINT": "Norėdami sukurti projektą, paleiskite 'ST_Setup.exe'", "CLOSING": "Užsidaro...",
    ...
    "ukr": {
        ...
        "NEW_PROJ": "[N] Створити новий проект (Setup)", "QUIT": "[Q] Вихід", "SELECT": "👉 Оберіть проект", "INVALID": "Невірний вибір.", "CREATE_HINT": "Щоб створити проект, запустіть 'ST_Setup.exe'", "CLOSING": "Закриття...",
    ...
    "rus": {
        ...
        "NEW_PROJ": "[N] Создать новый проект (Setup)", "QUIT": "[Q] Выход", "SELECT": "👉 Выберите проект", "INVALID": "Неверный выбор.", "CREATE_HINT": "Чтобы создать проект, запустите 'ST_Setup.exe'", "CLOSING": "Закрытие...",
...
def project_selection_menu():
    ...
    if choice == 'n':
        print(f"\n{Fore.GREEN}{LANG[L_CODE]['CREATE_HINT']}{Fore.RESET}")
        print(LANG[L_CODE]['CLOSING']); time.sleep(2); sys.exit(0)
...
def print_ui(d):
    ...
    print(f"\033[2J\033[H{Fore.CYAN}🛠️  STALKER TRANSLATOR (v55.2){Fore.RESET}")
...
def process_file(filename, start_idx=0, whitelist=set()):
    ...
    if HAS_BS4:
        ...
        tags = soup.find_all('text'); parser_obj = soup
    else:
        parser_obj = RegexParser(content); tags = parser_obj.get_tags()
    ...
    while idx < len(tags):
        ...
        while True:
            ...
            if cmd == 'n': idx += 1; break
                ...
                if new_txt != lit_txt:
                    if HAS_BS4: tag.string = new_txt; save_triple_versions(parser_obj, filename, True)
                ...
                try:
                    ...
                    if HAS_BS4: tag.string = tr; save_triple_versions(parser_obj, filename, True)
